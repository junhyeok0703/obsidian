...
##### SQL 기초 문법
SELECT 
- 데이터베이스에서 데이터를 선택하는 명령어 시작 키워드 
- column_list : 어떤 열의 데이터를 가져올지 지정
- \\*\*\를 사용시 모든 열 선택가능**
...
![[Pasted image 20240730194724.png]]

Join~on
- 두개이상의 테이블 연결하여 데이터를 가져오는 방법
- inner ,left ,right,full join등 다양한 조인 방식 존재
- ![[Pasted image 20240730233706.png]]
- ![[Pasted image 20240730233718.png]]

WHERE
- 특정 조건에 맞는 데이터만 필터링
- 비교연산자와 논리연산자
- ![[Pasted image 20240730233740.png]]

GROUP BY 
- 하나 이상의 열을 기준으로 결과를 그룹핑 집계함수와 함께 사용
- HAVING  (그룹바이 했을때만 사용할수있는 필터링 함수)
- GROUP BY로 생성된 그룹을 집계함수에 대한 조건을 기반으로 필터링
- WHERE절과 유사하지만 그룹화된 데이터에 적용

ORDER BY
- 최종 결과를 하나 이상의 열을 기준으로 정렬
- 오름차순(ASC) 또는 내림차순(DESC) 지정가능 
- ![[Pasted image 20240730234110.png]]
